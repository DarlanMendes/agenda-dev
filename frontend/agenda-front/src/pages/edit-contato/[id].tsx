...
export default function editPessoa(props: Props) {
    ...
    const handleFileSelected = async (e: any) => {
        ...
        try {
            const response = await axios.post('http://localhost:3001/upload/', formData,{headers: {
                'Content-Type': 'multipart/form-data', // É importante definir o tipo de conteúdo correto
              },})
            if (response) {
                console.log(response)
                //setPessoa((previous) => ({ ...previous, photoUrl: URL.createObjectURL(file) }));
            }
        } catch (e) {
            alert(e)
        }
